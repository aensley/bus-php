...
if (!array_key_exists($short, $data)) {
  http_response_code(400);
  echo json_encode(
    ['status' => 'error', 'message' => 'Short URL not found', 'short' => $short],
    JSON_FORCE_OBJECT
  );
  exit;
}
...
try {
  ...
  setData($data);
  echo json_encode(
    ['status' => 'success', 'message' => 'Short URL deleted', 'short' => $short],
    JSON_FORCE_OBJECT
  );
} catch (Exception $e) {
  http_response_code(500);
  echo json_encode(
    ['status' => 'error', 'message' => 'Unable to delete Short URL', 'short' => $short],
    JSON_FORCE_OBJECT
  );
}
